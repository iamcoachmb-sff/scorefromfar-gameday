...
const defaultLibraries = {
  formation: [
    "CUT DBL", "CUT TRIPLE", "CUT TRIPLE LT", "DBL", "DBL LT", "DEUCE", "DEUCE LT", "DEUCE LT YO", "DEUCE YO",
    "DIRTY", "DIRTY LT", "DUNK", "DUNK LT", "TRAIL LT", "TRAIN", "TRAIN LT", "TREY", "TREY LT", "TRIAL",
    "TRIO", "TRIO LT", "TRIPLE", "TRIPLE LT", "TROUBLE", "TROUBLE LT", "TRUCK", "TRUCK LT"
  ],
  motion: ["X", "H", "Y", "Z", "XIN", "ZIP", "HAC", "YAC", "H-ORB", "WAVE"],
  protection: ["50", "51", "70", "71", "350", "351", "360", "361", "816", "817", "850", "851", "BT 16", "BT 17", "LUCY", "RICKY"],
  play: [
    "16", "17", "10 CAB", "11 CAB", "12 WRAP", "13 WRAP", "16 BROOM", "16 CAB", "16 OAK", "16 TOSS",
    "17 BROOM", "17 CAB", "17 OAK", "17 REWIND PASS", "17 TOSS", "BREAK", "DIME TIDE", "FLOOD", "GOOSE SLUG",
    "JAIL", "MESH", "MESH TIDE", "NAKED", "OASIS H-STICK", "PEEL", "PUMP HAWK", "PUNCH 16", "PUNCH 17",
    "QTR 12 WRAP", "QTR 13 WRAP", "RAT 14", "RAT 15"
  ],
...
function aggregateTopPlays(plays, type, dimension) {
  ...
  return Array.from(grouped.values())
    .map((item) => {
      const sortedDimensions = Object.entries(item.dimensionCounts).sort((a, b) => b[1] - a[1] || String(a[0]).localeCompare(String(b[0])));
    ...
    .sort((a, b) => b.successRate - a.successRate || b.attempts - a.attempts || b.yards - a.yards || String(a.play).localeCompare(String(b.play)))
    .slice(0, 3);
}
...
function KeyButton({ children, className, active = false, tone = "default", onClick }) {
  return (
    <Button
      variant="outline"
      onClick={onClick}
...
function PlaylistColumn({ label, items, selectedValue, onSelect, tall = false }) {
  return (
    <div className="rounded-xl border border-zinc-200 bg-white shadow-sm">
      <div className="border-b border-zinc-100 px-3 py-2 text-[11px] font-semibold uppercase tracking-wide text-zinc-400">{label}</div>
  ...
  return (
    <Card className="rounded-2xl border-zinc-300 shadow-sm">
      <CardContent className="p-2">
        ...
        />
        <Button size="sm" className="mb-2 w-full" onClick={onSave}>Save {label}</Button>
        <div className="max-h-[300px] space-y-1 overflow-y-auto pr-1">
          {items.length ? items.map((item) => (
            <div key={item} className="flex items-center gap-2 rounded-lg border border-zinc-200 bg-zinc-50 px-2 py-1">
              ...
              <Button variant="outline" size="sm" onClick={() => onDelete(item)}>Delete</Button>
            </div>
        ...
        <CardContent className="p-4">
          <div className="flex items-center justify-between gap-3">
            <div>
              ...
              <div className="text-sm text-zinc-500">Paste or type one value per line in each category column, save it, and delete values directly from the column list.</div>
            </div>
            <div className="flex gap-2">
              <Badge>{Object.values(libraries).reduce((sum, values) => sum + values.length, 0)} items</Badge>
              <Button variant="outline" onClick={exportLocalCallSheet}>Export CSV</Button>
            </div>
          </div>
        </CardContent>
      </Card>
...
function MainDashboard({ libraries, onOpenReports, onOpenPlaylist, onOpenSettings, onPrintReports }) {
  ...
  function normalizePlay(data) {
    ...
    const isTdResult = normalizedResult === "touchdown" || normalizedResult === "rush td" || normalizedResult === "complete, td" || normalizedResult === "complete td";
    if (isTdResult) {
  ...
  function commitPlay() {
    ...
    const normalizedResult = String(play.result || "").trim().toLowerCase();
    const isTouchdown = normalizedResult === "touchdown" || normalizedResult === "rush td" || normalizedResult === "complete, td" || normalizedResult === "complete td";
    const isTurnover = normalizedResult === "interception" || normalizedResult === "fumble" || normalizedResult === "fumble, lost" || normalizedResult === "lost" || normalizedResult === "turnover";
  ...
  function exportCsv() {
    const headers = [
      "playNumber", "quarter", "series", "sequence", "down", "distance", "ballOnInternal", "ballOnDisplay",
      "hash", "playType", "formation", "motion", "protection", "play", "runConcept", "passConcept", "concept", "front", "blitz", "coverage", "result",
      "yards", "success", "driveId", "driveResult"
    ];
        ...
        <div className="grid grid-cols-12 gap-4">
          <div className="col-span-3">
            <div className="grid grid-cols-4 gap-3">
              {[
                ...
                if (key === "ADD PLAY") {
                  return (
                    <KeyButton key={i} tone="action" className="col-span-1 row-span-2 h-full min-h-[172px] text-2xl" onClick={commitPlay}>
                      ADD
                      <br />
                      PLAY
                    </KeyButton>
                ...
                if (key === "-25") return <KeyButton key={i} tone="danger" onClick={() => { setBallOnEntry("-25"); updateField("ballOn", 25); }}>{key}</KeyButton>;
          ...
          <Card className="col-span-4 rounded-2xl border-zinc-500 bg-gradient-to-br from-zinc-700 via-zinc-900 to-zinc-700 text-white shadow-2xl">
            <CardContent className="p-6">
              ...
              <div className="mt-6 flex items-center justify-center gap-8 text-2xl font-bold">
                <div>RUN: <Badge className="ml-2 text-2xl">{summary.run}</Badge></div>
                <div>PASS: <Badge className="ml-2 text-2xl">{summary.pass}</Badge></div>
              </div>
            </CardContent>
          </Card>
          ...
          <div className="col-span-1 flex flex-col gap-3">
            {hashOptions.map((side) => (
              <KeyButton key={side} tone="accent" active={form.hash === side} className="h-[106px] text-5xl" onClick={() => updateField("hash", side)}>
                {side}
              </KeyButton>
            ))}
          </div>
          ...
          <div className="col-span-4">
            ...
            <div className="grid grid-cols-[3fr_1fr] gap-3">
              <div className="grid grid-cols-3 gap-3">
                ...
                <KeyButton className="col-span-3 h-20" onClick={() => appendYardsDigit("0")}>0</KeyButton>
              </div>
              <div className="grid grid-rows-[1fr_1fr_2fr] gap-3">
                <div className="rounded-xl border border-zinc-300 bg-white p-3">
                  ...
                  <Input value={form.yards} readOnly className="mt-2 h-16 text-2xl" onClick={clearYards} />
                </div>
                <KeyButton className="h-full text-xl" onClick={toggleYardsNegative}>-</KeyButton>
                <KeyButton tone="action" className="h-full text-4xl disabled:opacity-50" onClick={commitPlay} disabled={!form.hash || form.yards === "" || form.yards === null || form.yards === undefined || (!form.runConcept && !form.passConcept) || !form.result}>GO</KeyButton>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="mt-4 grid grid-cols-1 gap-4 xl:grid-cols-[1.2fr_1fr]">
          <div className="rounded-xl border border-zinc-200 bg-white shadow-sm">
            ...
            <div className="h-[260px] overflow-y-auto px-2 py-2">
              <div className="space-y-1">
                {libraries.result.length ? libraries.result.map((item) => {
...
function ReportsDashboard({ plays }) {
  ...
  const efficiencyRows = useMemo(() => {
    ...
    return Array.from(grouped.values()).sort((a, b) => Number(a.down) - Number(b.down) || String(a.bucket).localeCompare(String(b.bucket)));
  }, [plays]);
          ...
          <div className="overflow-x-auto rounded-xl border border-zinc-200 bg-white">
            <table className="min-w-full text-left text-sm">
              ...
              <tbody>
                ...
                )) : <tr><td className="p-3 text-zinc-400" colSpan={5}>No data yet.</td></tr>}
              </tbody>
            </table>
          </div>
        </CardContent>
          ...
          <CardContent className="p-4">
            ...
            <div className="overflow-x-auto rounded-xl border border-zinc-200 bg-white">
              <table className="min-w-full text-left text-sm">
                ...
                <tbody>
                  {efficiencyRows.length ? efficiencyRows.map((item, idx) => (
                  ...
                  )) : <tr><td className="p-3 text-zinc-400" colSpan={9}>No efficiency data yet.</td></tr>}
                </tbody>
              </table>
            </div>
          </CardContent>
          ...
          <CardContent className="p-4">
            ...
            <div className="overflow-x-auto rounded-xl border border-zinc-200 bg-white">
              <table className="min-w-full text-left text-sm">
                ...
                <tbody>
                  {seriesRows.length ? seriesRows.map((item) => (
                  ...
                  )) : <tr><td className="p-3 text-zinc-400" colSpan={5}>No series data yet.</td></tr>}
                </tbody>
              </table>
            </div>
          </CardContent>
  ...
  return (
    <div className="min-h-screen bg-zinc-100 p-4 text-zinc-900">
      <div className="mx-auto max-w-[1700px] space-y-4">
        ...
        <div className="flex gap-2">
          <Button variant={activeScreen === "manager" ? "default" : "outline"} onClick={() => setActiveScreen("manager")}>Call Sheet Manager</Button>
          <Button variant={activeScreen === "dashboard" ? "default" : "outline"} onClick={() => setActiveScreen("dashboard")}>Main Dashboard</Button>
          <Button variant={activeScreen === "reports" ? "default" : "outline"} onClick={() => setActiveScreen("reports")}>Reports</Button>
        </div>
